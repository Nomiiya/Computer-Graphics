...
// matrix for shadow projection
	...
	// DCH code 
	console.log("extras draw flag = ",extradraw);
	console.log("light on y-axis = ",light[1]);
	console.log("shadow projection matrix = ",m);
	...
	// DCH: light starts here when theta=0
    pointsArray.push(vec4( 0.0, light[1], 1.0, 1.0 ));	
...
var render = function() {
		...
		// DCH: draw positive (x,y) point's projection
		if(extradraw == 1) {
			gl.uniform4fv(fColor, flatten(cyan));
			gl.drawArrays(gl.POINTS, 2, 1);
		};
		
		// rotate the light and display --- added by DCH
		// theta is in radian; rotate takes degrees
		modelViewMatrix = mvSave;
		modelViewMatrix = mult(modelViewMatrix, rotate(theta*(180.0/Math.PI), axis));
		gl.uniformMatrix4fv( modelViewMatrixLoc, false, flatten(modelViewMatrix) );
		
        gl.uniform4fv(fColor, flatten(yellow));
        gl.drawArrays(gl.POINTS, 4, 1);
		
		// DCH: Light line from light to last point on square
		if(extradraw == 1) {
